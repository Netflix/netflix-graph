...
public class NFWeightedCompressedGraphTest {
    ...
    private NFWeightedCompressedGraph compressedGraph;

    @Before
    public void setUp() {
        NFGraphSpec spec = new NFGraphSpec(new NFNodeSpec("a", new NFPropertySpec("multiple", "b", GLOBAL | MULTIPLE | COMPACT), new NFPropertySpec("single", "b", GLOBAL | SINGLE | COMPACT)), new NFNodeSpec("b"));

        NFWeightedBuildGraph graph = new NFWeightedBuildGraph(spec);

        graph.addConnection(NODE_TYPE, 0, MULTIPLE_PROPERTY, 0, 1, 1);
        graph.addConnection(NODE_TYPE, 0, MULTIPLE_PROPERTY, 0, 1);
        graph.addConnection(NODE_TYPE, 0, MULTIPLE_PROPERTY, 0, 1, 2);

        graph.addConnection(NODE_TYPE, 0, MULTIPLE_PROPERTY, 1, 2);
        graph.addConnection(NODE_TYPE, 0, MULTIPLE_PROPERTY, 1, 2, 2);
        graph.addConnection(NODE_TYPE, 0, MULTIPLE_PROPERTY, 1, 2);

        graph.addConnection(NODE_TYPE, 0, MULTIPLE_PROPERTY, 2, 20);
        graph.addConnection(NODE_TYPE, 0, MULTIPLE_PROPERTY, 3, 50, 2);

        graph.addConnection(NODE_TYPE, 0, SINGLE_PROPERTY, 0, 1, 1);
        graph.addConnection(NODE_TYPE, 0, SINGLE_PROPERTY, 0, 1);
        graph.addConnection(NODE_TYPE, 0, SINGLE_PROPERTY, 0, 1, 2);
    ...
    public void returnsValidOrdinalSetForSingleConnections() {
        WeightedOrdinalSet set = compressedGraph.getWeightedConnectionSet(NODE_TYPE, 0, SINGLE_PROPERTY);
    ...
    public void returnsValidOrdinalIteratorForSingleConnections() {
        WeightedOrdinalIterator iter = compressedGraph.getWeightedConnectionIterator(NODE_TYPE, 0, SINGLE_PROPERTY);
    ...
    public void returnsValidOrdinalIteratorForMultipleConnectionsWithCopy() {
        WeightedOrdinalIterator iter = compressedGraph.getWeightedConnectionIterator(NODE_TYPE, 0, MULTIPLE_PROPERTY);
        Assert.assertEquals(0, iter.nextOrdinal());
    ...
    public void returnsValidOrdinalSetForMultipleConnections() {
        WeightedOrdinalSet set = compressedGraph.getWeightedConnectionSet(NODE_TYPE, 0, MULTIPLE_PROPERTY);
        int[][] ordinalWithWeightAndProperty = set.asArrayWithWeightAndProperty();
        int[][] ordinalWithWeight = set.asArrayWithWeight();
        int[][] ordinalWithProperty = set.asArrayWithProperty();
    ...
    public void returnsFirstOrdinalForMultipleConnections() {
        int ordinal = compressedGraph.getConnection(NODE_TYPE, 0, MULTIPLE_PROPERTY);
        Assert.assertEquals(0, ordinal);
    }

    @Test
    public void returnsNegativeOneForUndefinedConnections() {
        int ordinal = compressedGraph.getConnection(NODE_TYPE, 1, MULTIPLE_PROPERTY);
        Assert.assertEquals(-1, ordinal);
    }

    @Test
    public void returnsEmptySetForUndefinedConnections() {
        WeightedOrdinalSet set = compressedGraph.getWeightedConnectionSet(NODE_TYPE, 1, MULTIPLE_PROPERTY);
        Assert.assertEquals(0, set.size());
    }

    @Test
    public void returnsEmptyIteratorForUndefinedConnections() {
        WeightedOrdinalIterator iter = compressedGraph.getWeightedConnectionIterator(NODE_TYPE, 1, MULTIPLE_PROPERTY);
        Assert.assertEquals(NO_MORE_ORDINALS, iter.nextOrdinal());
        iter.reset();
        int[] ordinalWithWeightAndProperty = iter.nextOrdinalWithWeightAndProperty();
    ...
    public void throwsNFGraphExceptionWhenQueryingForUndefinedNodeType() {
        try {
            compressedGraph.getWeightedConnectionSet("undefined", 0, MULTIPLE_PROPERTY);
    ...
    public void throwsNFGraphExceptionWhenQueryingForUndefinedProperty() {
        try {
            compressedGraph.getWeightedConnectionIterator(NODE_TYPE, 0, "undefined");
            Assert.fail("NFGraphException should have been thrown");
        } catch (NFGraphException expected) {
        }
    }
